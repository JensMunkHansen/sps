  ...
  fftwf_plan& Forward(size_t size, float* in, std::complex<float>* out)
  {
    ...
    else
    {
      ...
      if (Signal1DPlan<float>::measure)
      {
        ...
        forward[index] = fftwf_plan_dft_r2c_1d(
          static_cast<int>(size), dummy, reinterpret_cast<fftwf_complex*>(out), SPS_FFTW_ACCURATE);
        _mm_free(dummy);
      }
      else
      {
        forward[index] = fftwf_plan_dft_r2c_1d(
          static_cast<int>(size), in, reinterpret_cast<fftwf_complex*>(out), SPS_FFTW_FAST);
      }
      return forward[index];
    }
  }
  ...
  fftwf_plan& Backward(size_t size, std::complex<float>* in, float* out)
  {
    ...
    else
    {
      ...
      if (Signal1DPlan<float>::measure)
      {
        ...
        backward[index] = fftwf_plan_dft_c2r_1d(
          static_cast<int>(size), reinterpret_cast<fftwf_complex*>(dummy), out, SPS_FFTW_ACCURATE);
        _mm_free(dummy);
      }
      else
      {
        backward[index] = fftwf_plan_dft_c2r_1d(
          static_cast<int>(size), reinterpret_cast<fftwf_complex*>(in), out, SPS_FFTW_FAST);
      }
      return backward[index];
    }
  }
...
public:
  ...
  */
  ...
  fftw_plan& Forward(size_t size, double* in, std::complex<double>* out)
  {
    ...
    {
      std::lock_guard<std::mutex> guard(g_plan_mutex);
      forward[index] = fftw_plan_dft_r2c_1d(
        static_cast<int>(size), in, reinterpret_cast<fftw_complex*>(out), SPS_FFTW_FAST);
      return forward[index];
    }
  }
  ...
  fftw_plan& Backward(size_t size, std::complex<double>* in, double* out)
  {
    ...
    {
      std::lock_guard<std::mutex> guard(g_plan_mutex);
      backward[index] = fftw_plan_dft_c2r_1d(
        static_cast<int>(size), reinterpret_cast<fftw_complex*>(in), out, SPS_FFTW_FAST);
      return backward[index];
    }
  }
...
void DivideArray<float>(float* Data, size_t NumEl, float Divisor)
{
  ...
  float* aligned_ptr = reinterpret_cast<float*>(
    (reinterpret_cast<uintptr_t>(Data) + align - 1) & mask); // mask is 0x0F

  // Offset before first aligned address
  size_t offset = static_cast<size_t>(
    (reinterpret_cast<uintptr_t>(aligned_ptr) - reinterpret_cast<uintptr_t>(Data)) / sizeof(float));
...
void DivideArray<double>(double* Data, size_t NumEl, double Divisor)
{
  ...
  double* aligned_ptr =
    reinterpret_cast<double*>((reinterpret_cast<uintptr_t>(Data) + align - 1) & mask);

  int n = 0;

  size_t offset = static_cast<size_t>(
    (reinterpret_cast<uintptr_t>(aligned_ptr) - reinterpret_cast<uintptr_t>(Data)) /
    sizeof(double));
  ...
  // Shouldn't it be
  nCount =
    static_cast<int>((align / sizeof(double)) * ((NumEl - offset) / (align / sizeof(double))));
  ...
  nCount = static_cast<int>(NumEl) - static_cast<int>(align / sizeof(double) - 1) -
...
template <class T>
signal1D<T>::signal1D(size_t n, size_t bytes)
  : data(NULL)
  , offset(0)
  , ndata(n)
  , nbytes(0)
...
template <>
bool fft<float>(
  const sps::signal1D<float>& a, const size_t& n, sps::signal1D<std::complex<float>>& c)
{
...
template <>
bool ifft<float>(
  const sps::signal1D<std::complex<float>>& a, const size_t& n, sps::signal1D<float>& c)
{
...
template <>
bool fft<double>(
  const sps::signal1D<double>& a, const size_t& n, sps::signal1D<std::complex<double>>& c)
{
...
template <>
bool ifft<double>(
  const sps::signal1D<std::complex<double>>& a, const size_t& n, sps::signal1D<double>& c)
{
...
template <>
bool conv_fft<float>(
  const sps::signal1D<float>& a, const sps::signal1D<float>& b, sps::signal1D<float>& c)
{
  ...
  while (retval)
  {
    ...
    fft_a =
      static_cast<fftwf_complex*>(SPS_MM_MALLOC(sizeof(fftwf_complex) * ((n / 2 + 1) + 1), 16));
    fft_b =
      static_cast<fftwf_complex*>(SPS_MM_MALLOC(sizeof(fftwf_complex) * ((n / 2 + 1) + 1), 16));
...
template <>
bool conv_fft<double>(
  const sps::signal1D<double>& a, const sps::signal1D<double>& b, sps::signal1D<double>& c)
{
...
template <class T>
msignal1D<T>::msignal1D(size_t n)
  : msignal1D(n, 16 * (n * sizeof(T) + 15) / 16)
{
}
...
template <class T>
msignal1D<T>::msignal1D(size_t _ndata, size_t _nbytes)
  : offset(0)
  , ndata(_ndata)
  , nbytes(16 * (std::max<size_t>(_ndata * sizeof(T), _nbytes) + 15) / 16)
  , m_data(NULL)
{
  m_data.reset(static_cast<T*>(_mm_malloc(nbytes, 16)),
    [=](T* p)
    {
      _mm_free(p);
      p = NULL;
...
void msignal1D<T>::pad(size_t _nbytes)
{
  if ((nbytes > 0) && (_nbytes > nbytes))
  {
    // Shallow copy
    std::shared_ptr<T> data1 = m_data;
    m_data.reset(static_cast<T*>(_mm_malloc(_nbytes, 16)),
      [=](T* p)
      {
        _mm_free(p);
        p = NULL;
...
template <typename T>
bool conv_fft(const sps::signal1D<T>& /* a */, sps::signal1D<T>& /* b */)
{
...
template <>
bool conv_fft_out_in(const signal1D<float>& a, signal1D<float>& b)
{
...
template <>
bool conv_fft_out_in(const signal1D<double>& a, signal1D<double>& b)
{
...
template <class T>
void conv_ansi(const T* Signal, size_t SignalLen, const T* Kernel, size_t KernelLen, T* Result)
{
  size_t n;
...
template <typename T>
bool conv(const signal1D<T>& a, const signal1D<T>& b, signal1D<T>& c)
{
...
bool mconv_fft(const msignal1D<T>& a, const msignal1D<T>& b, msignal1D<T>& c)
{
  ...
  {
    c.nbytes = nbytes;
    c.m_data = std::shared_ptr<T>(static_cast<T*>(_mm_malloc(c.nbytes, 16)),
      [=](T* p)
      {
        _mm_free(p);
        p = nullptr;
      ...
      if (b.ndata == 1)
      {
        ...
        else
        {
          if (a.ndata == 1)
          {
            ...
            c.nbytes = 16;
            c.offset = a.offset + b.offset;
            c.data = std::shared_ptr<T>((T*)_mm_malloc(c.nbytes, 16),
              [=](T* p)
              {
                _mm_free(p);
                p = nullptr;
...
bool mfft(const msignal1D<T>& a, const size_t& n, msignal1D<std::complex<T>>& c)
{
  ...
  if (c.nbytes < nbytes)
  {
    c.nbytes = nbytes;
    c.m_data =
      std::shared_ptr<std::complex<T>>(static_cast<std::complex<T>*>(_mm_malloc(c.nbytes, 16)),
        [=](std::complex<T>* p)
        {
          _mm_free(p);
          p = nullptr;
...
template bool SPS_EXPORT conv<float>(
  const signal1D<float>& a, const signal1D<float>& b, signal1D<float>& c);

template bool SPS_EXPORT conv<double>(
  const signal1D<double>& a, const signal1D<double>& b, signal1D<double>& c);
...
template bool SPS_EXPORT conv_fft_fs<float>(
  const float& fs, const signal1D<float>& a, const signal1D<float>& b, sps::signal1D<float>& c);

// template bool SPS_EXPORT conv_fft<double>(const sps::signal1D<double> &a,
//     const sps::signal1D<double> &b,
//     sps::signal1D<double>& c);

template bool SPS_EXPORT conv_fft_fs<double>(
  const double& fs, const signal1D<double>& a, const signal1D<double>& b, signal1D<double>& c);
...
template bool SPS_EXPORT mconv_fft<float>(
  const msignal1D<float>& a, const msignal1D<float>& b, msignal1D<float>& c);

template bool SPS_EXPORT mconv_fft<double>(
  const msignal1D<double>& a, const msignal1D<double>& b, msignal1D<double>& c);

template bool SPS_EXPORT mconv_fft<float>(
  const msignal1D<float>& a, const msignal1D<float>& b, signal1D<float>& c);

template bool SPS_EXPORT mconv_fft<double>(
  const msignal1D<double>& a, const msignal1D<double>& b, signal1D<double>& c);

template bool SPS_EXPORT mconv_fft_fs<float>(
  const float& fs, const msignal1D<float>& a, const msignal1D<float>& b, msignal1D<float>& c);

template bool SPS_EXPORT mconv_fft_fs<double>(
  const double& fs, const msignal1D<double>& a, const msignal1D<double>& b, msignal1D<double>& c);

// These are explicit specializations (not primary templates), instantiation has no effect
// template bool SPS_EXPORT fft<float>(const signal1D<float>& a, const size_t &n,
// signal1D<std::complex<float> >& c); template bool SPS_EXPORT fft<double>(const signal1D<double>&
// a, const size_t &n, signal1D<std::complex<double> >& c); template bool SPS_EXPORT
// ifft<float>(const signal1D<std::complex<float> >& a, const size_t &n, signal1D<float>& c);
// template bool SPS_EXPORT ifft<double>(const signal1D<std::complex<double> >& a, const size_t &n,
// signal1D<double>& c);

// pack_r2c and unpack_c2r are template functions, keep these
template bool SPS_EXPORT pack_r2c<double>(
  const signal1D<double>& a, signal1D<std::complex<double>>& c);
template bool SPS_EXPORT unpack_c2r<double>(
  const signal1D<std::complex<double>>& a, signal1D<double>& c);
template bool SPS_EXPORT pack_r2c<float>(
  const signal1D<float>& a, signal1D<std::complex<float>>& c);
template bool SPS_EXPORT unpack_c2r<float>(
  const signal1D<std::complex<float>>& a, signal1D<float>& c);

template bool SPS_EXPORT mfft<float>(
  const msignal1D<float>& a, const size_t& n, msignal1D<std::complex<float>>& c);
template bool SPS_EXPORT mfft<double>(
  const msignal1D<double>& a, const size_t& n, msignal1D<std::complex<double>>& c);
template bool SPS_EXPORT mifft<float>(
  const msignal1D<std::complex<float>>& a, const size_t& n, msignal1D<float>& c);
template bool SPS_EXPORT mifft<double>(
  const msignal1D<std::complex<double>>& a, const size_t& n, msignal1D<double>& c);
...
This routine plans multiple multidimensional complex DFTs, and it extends the fftw_plan_dft routine
(see Complex DFTs) to compute howmany transforms, each having rank rank and size n. In addition, the
transform data need not be contiguous, but it may be laid out in memory with an arbitrary stride. To
account for these possibilities, fftw_plan_many_dft adds the new parameters howmany, {i,o}nembed,
{i,o}stride, and {i,o}dist. The FFTW basic interface (see Complex DFTs) provides routines
specialized for ranks 1, 2, and 3, but the advanced interface handles only the general-rank case.

howmany is the number of transforms to compute. The resulting plan computes howmany transforms,
where the input of the k-th transform is at location in+k*idist (in C pointer arithmetic), and its
output is at location out+k*odist. Plans obtained in this way can often be faster than calling FFTW
multiple times for the individual transforms. The basic fftw_plan_dft interface corresponds to
howmany=1 (in which case the dist parameters are ignored).

Each of the howmany transforms has rank rank and size n, as in the basic interface. In addition, the
advanced interface allows the input and output arrays of each transform to be row-major subarrays of
larger rank-rank arrays, described by inembed and onembed parameters, respectively. {i,o}nembed must
be arrays of length rank, and n should be elementwise less than or equal to {i,o}nembed. Passing
NULL for an nembed parameter is equivalent to passing n (i.e. same physical and logical dimensions,
as in the basic interface.)

The stride parameters indicate that the j-th element of the input or output arrays is located at
j*istride or j*ostride, respectively. (For a multi-dimensional array, j is the ordinary row-major
index.) When combined with the k-th transform in a howmany loop, from above, this means that the
(j,k)-th element is at j*stride+k*dist. (The basic fftw_plan_dft interface corresponds to a stride
of 1.)

For in-place transforms, the input and output stride and dist parameters should be the same;
otherwise, the planner may return NULL.

Arrays n, inembed, and onembed are not used after this function returns. You can safely free or
reuse them.
