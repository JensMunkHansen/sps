...
// A branch on GCC version is introduced: Addresses which before were
// relative is now absolute addresses in virtual memory!!!
...
sps::STrace::straceErrorCodes STrace::Enable() {
  ...
  // Consider listening to floating errors also
...
sps::STrace::straceErrorCodes STrace::Disable() {
  if (m_bInitialized) {
    ...
    // TEST
    sigaction(SIGFPE, &m_sa_fpe, NULL);
  }
  return sps::STrace::straceErrorCodes(EXIT_SUCCESS);
}
...
char* STrace::addr2line(const char *image /* input exe name */,
  ...
  *memory += line_max_length;
  ...
  if (line[0] == '?') {
    ...
    // objdump -C -d ./a.out | grep testMe => 2d36
    // addr2line -e a.out 2d36 => strace_test.cpp:28
