...
extern "C"
{
   ...
   * @{
   */
  const ALIGN16_BEGIN int _clear_signmask[4] ALIGN16_END = { 0x7FFFFFFFL, 0x7FFFFFFFL, 0x7FFFFFFFL,
    0x7FFFFFFFL }; ///< clear sign mask for ps

  const ALIGN16_BEGIN long long int _clear_signmaskd[2] ALIGN16_END = { 0x7FFFFFFFFFFFFFFF,
    0x7FFFFFFFFFFFFFFF }; ///< clear sign mask for pd

  const ALIGN32_BEGIN long long int _clear_signmask_256[4] ALIGN32_END = { 0x7FFFFFFFFFFFFFFF,
    0x7FFFFFFFFFFFFFFF, 0x7FFFFFFFFFFFFFFF, 0x7FFFFFFFFFFFFFFF }; ///< clear sign mask for ps

#ifdef _WIN32
  const __m128 signmask =
    _mm_castsi128_ps(_mm_set1_epi32(0x80000000L)); ///< signmask for ps (packed singles)
#elif defined(__GNUC__)
const __m128 signmask =
  _mm_castsi128_ps(_mm_set1_epi32(-2147483648)); ///< signmask for ps (packed singles)
#endif
  ...
  }; ///< negative sign mask for pd

  const __m128i negmask = _mm_set1_epi32(-1); ///< negate mask 0xFFFFFFFF for epi32

  const __m256i negmask_256 =
    _mm256_set_epi64x(-1, -1, -1, -1); ///< negate mask 0xFFFFFFFFFFFFFFFF for epi32
  ...
  typedef union __declspec(intrin_type) _CRT_ALIGN(32) v8f
  {
     ...
     */
    ///@{
    v8f(const __m256& _v) { v = _v; }
    v8f(const float& f7, const float& f6, const float& f5, const float& f4, const float& f3,
  ...
  typedef union __declspec(intrin_type) _CRT_ALIGN(32) v8i
  {
    ...
    uint16_t uint16[16];
    uint32_t uint32[8];
    v8i() { v = _mm256_setzero_si256(); }
    /** @name Initializers useful for constant initializations
     *
     */
    ///@{
    v8i(const __m256i& _v) { v = _v; }
    v8i(const int& i7, const int& i6, const int& i5, const int& i4, const int& i3, const int& i2,
  ...
  typedef union __declspec(intrin_type) _CRT_ALIGN(32) v4d
  {
    __m256d v;
    double f64[4];
    v4d() { v = _mm256_setzero_pd(); }
    v4d(const __m256d& _v) { v = _v; }
  } v4d;
...
typedef union v8i
{
  ...
  /**
   * Enable exceptions: default is to clear all exceptions and mask all exceptions, except INVALID
   * and OVERFLOW.
   *
   * We stop on _MM_MASK_DIV_ZERO, _MM_MASK_UNDERFLOW, _MM_MASK_DENORM, _MM_MASK_INEXACT
   */
  STATIC_INLINE_BEGIN void _mm_enable_exceptions(
    int mask = _MM_MASK_MASK & ~(_MM_MASK_OVERFLOW | _MM_MASK_INVALID))
  {
    _mm_setcsr(mask);
    // Disable denormals
    // mask = mask | 0x8040;
...
#define _mm_setbits_si128(a) _mm_cmpeq_epi32(a, a) // 1 cycle ...
  ...
  vect_128_intrs_signt(__m128i) _mm_cmpge_epi32(__m128i a, __m128i b)
  {                                                 // 3 cycles ...
    return _mm_invert_si128(_mm_cmplt_epi32(a, b)); // a >= b == ~(a < b)
  }

  vect_128_intrs_signt(__m128i) _mm_cmple_epi32(__m128i a, __m128i b)
  {                                                 // 3 cycles ...
  ...
  STATIC_INLINE_BEGIN __m256d _mm256_movehl_pd(__m256d a, __m256d b)
  {
    return _mm256_castps_pd(
      _mm256_permute2f128_ps(_mm256_castpd_ps(a), _mm256_castpd_ps(b), 0x11 /*0x21*/));
  }
...
// AVX-512 (HACK)
...
#if !defined(INTEL_COMPILER) && !(defined(__clang__)) &&                                           \
  !(defined(_MSC_VER) && (_MSC_VER >= 1914)) && !(defined(__GNUC__) && __GNUC__ > 7)
  STATIC_INLINE_BEGIN double _mm256_cvtsd_f64(__m256d d)
  {
  ...
  STATIC_INLINE_BEGIN void _mm_mul_epi16_full(
    __m128i& hiResult, __m128i& loResult, const __m128i a, const __m128i b)
  {
    __m128i hi = _mm_mulhi_epi16(a,
      b); // (a7*b7[16:31],a6*b6[16:31],a5*b5[16:31],a4*b4[16:31],a3*b3[16:31],a2*b2[16:31],a1*b1[16:31],a0*b0[16:31])
    __m128i low =
      _mm_mullo_epi16(a, b); // (a7*b7[0:15] ,a6*b6[0:15] ,a5*b5[0:15] ,a4*b4[0:15],a3*b3[0:15]
                             // ,a2*b2[0:15] ,a1*b1[0:15] ,a0*b0[0:15])
    loResult = _mm_unpacklo_epi16(hi,
      low); // (a3*b3[0:15],a3*b3[16:31],a2*b2[0:15],a2*b2[16:31],a1*b1[0:15],a1*b1[16:31],a0*b0[0:15],a0*b0[16:31])
    hiResult = _mm_unpackhi_epi16(hi,
      low); // (a7*b7[0:15],a7*b7[16:31],a6*b6[0:15],a6*b6[16:31],a5*b5[0:15],a5*b5[16:31],a4*b4[0:15],a4*b4[16:31])
  }
#endif

  STATIC_INLINE_BEGIN void _mm_mul_epi32_full(
    __m128i& loResult, __m128i& hiResult, __m128i a, __m128i b)
  {
...
#define _mm_srli_ps(x, imm) _mm_castsi128_ps(_mm_srli_si128(_mm_castps_si128(x), imm))
#endif
...
#else
// 8-bit const immediates cannot be considered immediate when debugging
#define _mm_slli_ps(x, imm) _mm_castsi128_ps(_mm_slli_si128(_mm_castps_si128(x), imm))
#endif
  ...
  STATIC_INLINE_BEGIN __m256d _mm256_dp_pd(
    const __m256d& x, const __m256d& y, const int mask) STATIC_INLINE_END;
  ...
  STATIC_INLINE_BEGIN __m256d _mm256_dp_pd(const __m256d& x, const __m256d& y, const int mask)
  {
    ...
    const __m256d selectMask = _mm256_cmp_pd(
      _mm256_castsi256_pd(_mm256_and_si256(smask, bum)), _mm256_castsi256_pd(smask), _CMP_EQ_UQ);
    const __m256d outputMask = _mm256_cmp_pd(
      _mm256_castsi256_pd(_mm256_and_si256(omask, bum)), _mm256_castsi256_pd(omask), _CMP_EQ_UQ);
#else
  // Without AVX-2 more casts are needed
  const __m256d selectMask = _mm256_cmp_pd(
    _mm256_castps_pd(_mm256_and_ps(_mm256_castsi256_ps(smask), _mm256_castsi256_ps(bum))),
    _mm256_castsi256_pd(smask), _CMP_EQ_UQ);
  const __m256d outputMask = _mm256_cmp_pd(
    _mm256_castps_pd(_mm256_and_ps(_mm256_castsi256_ps(omask), _mm256_castsi256_ps(bum))),
    _mm256_castsi256_pd(omask), _CMP_EQ_UQ);
#endif
    ...
    __m256d temp_high_low = _mm256_castps_pd(
      _mm256_permute2f128_ps(_mm256_castpd_ps(temp), _mm256_castpd_ps(temp), 0x21));
...
#error Enable SSE2, SSE3 or SSE4
  ...
  {
    return _mm_and_pd(x, _mm_load_pd(reinterpret_cast<const double*>(_clear_signmaskd)));
  }

  STATIC_INLINE_BEGIN __m256d _mm256_fabs_pd(__m256d x)
  {
    return _mm256_and_pd(x, _mm256_load_pd(reinterpret_cast<const double*>(_clear_signmask_256)));
  }
  ...
  {
    return _mm_xor_pd(x, _mm_load_pd(reinterpret_cast<const double*>(_neg_signmaskd)));
  }
  ...
  STATIC_INLINE_BEGIN __m256d _mm256_sel_pd(__m256d a, __m256d b, __m256d mask)
  {
#ifdef HAVE_ZMMINTRIN_H
...
// To avoid signaling in SDK code, we set infinity = (2 - 2**(-23)) * 2**(254-127) =
// reinterpret_cast<float>(0x7F7FFFFF)
...
#pragma clang diagnostic pop
  ...
  STATIC_INLINE_BEGIN int8_t _mm_max1_epi8(__m128i buffer)
  {
    ...
    return static_cast<int8_t>(127 - _mm_cvtsi128_si32(tmp3));
#else
...
STATIC_INLINE_BEGIN __m128 _mm_fmadd_ps(__m128 a, __m128 b, __m128 c)
{
  ...
  STATIC_INLINE_BEGIN __m128 _mm_rsqrt_nr_ps(const __m128& a)
  {
    ...
    return _mm_add_ps(_mm_mul_ps(_mm_set1_ps(1.5f), r),
      _mm_mul_ps(_mm_mul_ps(_mm_mul_ps(a, _m_m_half_ps), r), _mm_mul_ps(r, r)));
  }
  ...
  STATIC_INLINE_BEGIN __m128 _mm_sqrt_zero_ps(const __m128& x)
  {
    ...
    return _mm_or_ps(_mm_and_ps(mask, sqrt), _mm_andnot_ps(mask, _mm_setzero_ps()));
  }
  ...
  {
    __m256d ul =
      _mm256_castps_pd(_mm256_permute2f128_ps(_mm256_castpd_ps(x), _mm256_castpd_ps(x), 0x21));
  ...
  */
  STATIC_INLINE_BEGIN void _mm_transpose_4x4_ps(const float a[4][4], float b[4][4])
  {
    __m128 row0, row1, row2, row3;
    row0 = _mm_load_ps(a[0]);
    row1 = _mm_load_ps(a[1]);
    row2 = _mm_load_ps(a[2]);
    row3 = _mm_load_ps(a[3]);
...
#define SPS_IGNORE_CONSTANT_CONDITION_END __pragma(warning(pop))
#else
#define SPS_IGNORE_CONSTANT_CONDITION_BEGIN
...
#define _MM_TRANSPOSE8_PS(row0, row1, row2, row3, row4, row5, row6, row7)                          \
  ...
  } while (0) SPS_IGNORE_CONSTANT_CONDITION_END

  STATIC_INLINE_BEGIN void _mm_transpose_8x8_ps(const float a[8][8], float b[8][8])
  {

    __m256 row0, row1, row2, row3, row4, row5, row6, row7;
    row0 = _mm256_load_ps(a[0]);
    row1 = _mm256_load_ps(a[1]);
    row2 = _mm256_load_ps(a[2]);
    row3 = _mm256_load_ps(a[3]);
    row4 = _mm256_load_ps(a[4]);
    row5 = _mm256_load_ps(a[5]);
    row6 = _mm256_load_ps(a[6]);
    row7 = _mm256_load_ps(a[7]);
...
//__AVX512PF__
  ...
  void _mm_transpose_16x16_epi32(const int a[16][16], int b[16][16])
  {
...
inline std::ostream& operator<<(std::ostream& out, __m64 vi)
{
  ...
  _mm_stream_pi(reinterpret_cast<__m64*>(&output[0]), vi);
  out << "__m64[3]: " << output[3] << " __m64[2]: " << output[2] << " __m64[1]: " << output[1]
      << " __m64[0]: " << output[0] << std::endl;
#endif
  return out;
}
#endif
...
#if 1
#define SPS_SPLATS(x) _mm_set1_ps(x)
#else
#define SPS_SPLATS(x) _mm_broadcast_ss((float*)&x)
#endif
