...
class alignas(64) DynAllocators {
 public:
  DynAllocators() {
    debug_print("\n");
  }
  ...
  void operator delete(void* p) {
    debug_print("unaligned\n");
    DynAllocators* pObject = reinterpret_cast<DynAllocators*>(p);
  ...
  void* operator new( std::size_t size, std::align_val_t al) {
    debug_print("aligned\n");
#ifdef _MSC_VER
    DynAllocators* ptr = (DynAllocators*) _aligned_malloc(size, static_cast<std::size_t>(al));
    new (ptr) DynAllocators();  // Calls placement new
    return ptr ? ptr : throw std::bad_alloc{};
#else
    DynAllocators* ptr = (DynAllocators*) _mm_malloc(size, static_cast<std::size_t>(al));
    return ptr ? ptr : throw std::bad_alloc{};
#endif
  }
  void operator delete(void* ptr, std::size_t size, std::align_val_t align) {
    debug_print("aligned\n");
    DynAllocators* pObject = reinterpret_cast<DynAllocators*>(ptr);
    pObject->~DynAllocators();
#ifdef _MSC_VER
    _aligned_free(ptr);
#else
    _mm_free(ptr);
...
#if 0
    // Why is this wrong
    DynAllocators* ptr = (DynAllocators*)_aligned_malloc(count, static_cast<std::size_t>(al));
    size_t nObjects = (count - static_cast<std::size_t>(al)) / sizeof(DynAllocators);
    new (ptr) DynAllocators[nObjects]; // Calls placement new
  ...
  void operator delete[](void* ptr, std::size_t count, std::align_val_t al) {
    ...
    // Why is this wrong
    size_t nObjects = (count - static_cast<std::size_t>(al)) / sizeof(DynAllocators);
    DynAllocators* pObject = reinterpret_cast<DynAllocators*>(reinterpret_cast<uintptr_t>(ptr) + static_cast<std::size_t>(al));
...
int main() {
  printf("%d\n", __cplusplus);
  DynAllocators* d = new DynAllocators();
  delete d;
  return 0;
}
