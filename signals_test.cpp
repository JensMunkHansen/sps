...
TEST(signals_test, test_conv_float)
{
  ...
  assert((fmax_diff / (2 * next_power_two<size_t>(nc)) < 1.1 * FLT_EPSILON) &&
    "conv_fft too inaccurate");
...
TEST(signals_test, test_conv_double)
{
  ...
  assert((dmax_diff / (2 * next_power_two<size_t>(nc)) < 1.1 * DBL_EPSILON) &&
    "conv_fft too inaccurate");
...
TEST(signals_test, test_fft)
{
  ...
  assert(
    (fmax_diff / (2.0 * next_power_two<size_t>(n)) < 1.1 * FLT_EPSILON) && "fft too inaccurate");
