...
template <typename T>
struct is_comparable<T, decltype(std::declval<T&>() == std::declval<T&>())> : std::true_type
{
};
template <typename T>
...
struct member_pointer_class<F T::*>
{
  using type = T;
};
template <typename F>
...
template <typename F, typename T>
struct member_pointer_value<F T::*>
{
  using type = F;
};
template <typename F>
...
protected:
  ...
  command(connection* conn, slot_invoker call, self_deleter dtor) noexcept;
...
protected:
  ...
  target(slot_invoker call, cmnd_creator ctor, const vtbl* vptr) noexcept;

  template <typename>
  friend const vtbl* target_vtable() noexcept;
...
{
  command_imp(connection* conn, slot_invoker call, self_deleter dtor, local_args<Ts...>* args);
...
inline void command_invoke(command* self, target* trgt)
{
  ...
  std::apply(static_cast<trgt_t*>(trgt)->slot_, static_cast<self_t*>(self)->pack_);
}
...
template <typename Trgt>
inline bool target_compare(
  [[maybe_unused]] const target* lhs, [[maybe_unused]] const target* rhs) noexcept
{
  if constexpr (is_comparable_v<decltype(Trgt::slot_)>)
    return static_cast<const Trgt*>(lhs)->slot_ == static_cast<const Trgt*>(rhs)->slot_;
  else
    return false;
}
...
inline void target_invoke(target* self, void* args)
{
  ...
  std::apply(static_cast<self_t*>(self)->slot_, *static_cast<pack_t*>(args));
}
...
inline command* target_create(connection* conn, void* args)
{
  ...
  return memory_new<cmnd_t>(
    conn, &command_invoke<self_t>, &command_delete<cmnd_t>, static_cast<pack_t*>(args));
}
...
template <typename... Ts>
command_imp<Ts...>::command_imp(
  connection* conn, slot_invoker call, self_deleter dtor, local_args<Ts...>* args)
...
template <typename Fn, typename... Ts>
template <typename F>
target_imp<Fn, Ts...>::target_imp(F&& func)
  : target{ &target_invoke<target_imp>, &target_create<target_imp>, target_vtable<target_imp>() }
...
template <typename... ArgTs>
template <typename Ty, typename Fn>
unsigned signal<void(ArgTs...)>::connect(Ty* object, Fn slot, exec type)
{
  ...
  if constexpr (std::is_member_pointer_v<Fn>)
  {
    ...
    {
      static_assert(
        std::is_invocable_v<Fn, Ty*, ArgTs...>, "signal::connect: signal-slot type mismatch");
    }
    else
    {
      static_assert(std::is_invocable_v<ns::member_pointer_value_t<Fn>, ArgTs...>,
        "signal::connect: signal-slot type mismatch");
    }
  ...
  {
    static_assert(
      std::is_base_of_v<context, Ty>, "signal::connect: class does not inherit from 'context'");
    static_assert(std::is_invocable_v<Fn, ArgTs...>, "signal::connect: signal-slot type mismatch");
...
template <typename... ArgTs>
template <typename Fn>
unsigned signal<void(ArgTs...)>::connect(Fn slot)
{
  namespace ns = vdk::internal::signals;
  static_assert(std::is_invocable_v<Fn, ArgTs...>, "signal::connect: signal-slot type mismatch");
  if constexpr (std::is_pointer_v<Fn>)
...
template <typename... ArgTs>
template <typename Ty, typename Fn>
bool signal<void(ArgTs...)>::disconnect(Ty* object, Fn slot) noexcept
{
  namespace ns = vdk::internal::signals;

  static_assert(std::is_member_pointer_v<Fn>, "signal::disconnect: slot is not a member pointer");
  ...
  {
    static_assert(
      std::is_invocable_v<Fn, Ty*, ArgTs...>, "signal::disconnect: signal-slot type mismatch");
  }
  else
  {
    static_assert(std::is_invocable_v<ns::member_pointer_value_t<Fn>, ArgTs...>,
...
template <typename... ArgTs>
template <typename Fn>
bool signal<void(ArgTs...)>::disconnect(Fn slot) noexcept
{
  namespace ns = vdk::internal::signals;
  static_assert(std::is_invocable_v<Fn, ArgTs...>, "signal::disconnect: signal-slot type mismatch");
  if constexpr (std::is_pointer_v<Fn>)
    if (!slot)
      return false;
  ns::target_imp<Fn, ArgTs...> trgt{ std::move(slot) };
...
template <typename... ArgTs>
bool signal<void(ArgTs...)>::disconnect(unsigned id) noexcept
{
  if (!id)
    return false;
...
template <typename... ArgTs>
void signal<void(ArgTs...)>::disconnect() noexcept
{
...
template <typename F>
target_imp<Fn, Ts...>::target_imp(F&& func)
  : target{ &target_invoke<target_imp>, nullptr, target_vtable<target_imp>() }
  , slot_{ std::forward<F>(func) }
{
}
...
template <typename... ArgTs>
template <typename Ty, typename Fn>
unsigned signal<void(ArgTs...)>::connect(Ty* object, Fn slot)
{
  namespace ns = vdk::internal::signals;
