...
#define SZ 1024
  ...
  out << c2 << "This is red and underlined" << sps::set_color_s(&parr, nLeft, sps::colors::none)
      << std::endl;

  out << "0x" << std::setfill('0') << std::hex << std::setw(2 * sizeof(uintptr_t)) << (uintptr_t)&f
      << std::endl;

  printf("bytes left: %lu\n", nLeft);
  /*
  using namespace sps;
  out << set_color(colors::red, colors::cyan) << set_face(faces::uline) << "This is red" <<
  set_color(colors::none) << std::endl;
  */
