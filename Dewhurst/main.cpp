...
//
constexpr auto isInterfaceClass = isPtrSize & hasVirtualDestructor & isAbstract;
...
{
  static_assert(
    satisfies<T1, T2, T3>(isInterfaceClass), "template arguments must be interface classes.");
  // implementation...
};
...
{
  static_assert(
    satisfies<Ts...>(isInterfaceClass), "template arguments must be interface classes.");
  // implementation...
};
...
public:
  ...
  void doit(S&& a)
  {
    std::cout << "What::doit(universal): " << typeid(S).name() << std::endl;
  }
  ...
  What(In b, In e)
  {
    std::cout << "What::What(In, In)" << std::endl;
  }

  // Note that we have to add trailing default to allow member overload!
  // Here we're using Ignore, but any type will do.
  template <typename Rand, typename = Constraint<decltype(isRand & !isPointer), Rand>,
    typename = Ignore<1>>
  What(Rand b, Rand e)
  {
    std::cout << "What::What(Rand, Rand)" << std::endl;
  }

  template <typename Ptr, typename = Constraint<IsPointer, Ptr>, typename = Ignore<1>,
    typename = Ignore<2>>
  What(Ptr b, Ptr e)
  {
    std::cout << "What::What(Ptr, Ptr)" << std::endl;
  }
  ...
  void f(In b, In e) const
  {
    std::cout << "What::f(In, In)" << std::endl;
  }
  ...
  void f(Rand b, Rand e) const
  {
    std::cout << "What::f(Rand, Rand)" << std::endl;
  }

  template <typename Ptr, bool = constraint<Ptr>(isPointer), bool = ignore(1)>
  void f(Ptr b, Ptr e) const
  {
    std::cout << "What::f(Ptr, Ptr)" << std::endl;
  }
  ...
  static constexpr auto cast_constraint =
    isInterfaceClass & bind1st<std::is_base_of, Shape>() & !bind2nd<std::is_same, Shape>();
  using CastConstraint = decltype(cast_constraint);
  template <typename Target, typename = Constraint<CastConstraint, Target>>
  Target* cast_it()
  {
...
void misc_tests()
{
  ...
  cout << "Triangle is interface (false): " << boolalpha << satisfies<Triangle>(isInterfaceClass)
       << endl;
  ...
  constexpr auto too_complex = (isClass & (isPod | !isPolymorphic) ^ isUnsigned & !isTrivial) |
    (isPtrSize & !isFloatingPoint) | bind2nd<is_of_size, 8>();
