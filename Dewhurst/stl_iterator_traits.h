...
template <typename It>
using is_exactly_rand = std::is_same<Category<It>, std::random_access_iterator_tag>;
constexpr auto isExactlyRand = pred<is_exactly_rand>();

template <typename It>
using is_exactly_bi = std::is_same<Category<It>, std::bidirectional_iterator_tag>;
constexpr auto isExactlyBi = pred<is_exactly_bi>();

template <typename It>
using is_exactly_for = std::is_same<Category<It>, std::forward_iterator_tag>;
constexpr auto isExactlyFor = pred<is_exactly_for>();

template <typename It>
using is_exactly_in = std::is_same<Category<It>, std::input_iterator_tag>;
constexpr auto isExactlyIn = pred<is_exactly_in>();

template <typename It>
using is_exactly_out = std::is_same<Category<It>, std::output_iterator_tag>;
