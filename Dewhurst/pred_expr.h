...
namespace DandS {
	inline namespace ASTImplementation {
		...
		struct AllTrue<b, bs...> { enum : bool { value = b && AllTrue<bs...>::value }; };

		template <>
		struct AllTrue<> { enum : bool { value = true }; };
	...
	struct BindT1st : PAST {
		...
		static constexpr bool eval() { return BPred<First, Second>::value; }
	};
	template <template <typename, typename> class BPred, typename First>
	...
	template <typename P1, typename P2, typename = all_PAST<P1, P2>>
	constexpr auto operator |(P1, P2) { return Or<P1, P2>(); }
	...
	template <typename P1, typename P2, typename = all_PAST<P1, P2>>
	constexpr auto operator ^(P1, P2) { return Xor<P1, P2>(); }
