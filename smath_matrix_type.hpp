  ...
  SPS_EXPORT mat3_t
{
  ...
  mat3_t() { memset(this->data, 0, 9 * sizeof(T)); }
  mat3_t(const T& e00, const T& e01, const T& e02, const T& e10, const T& e11, const T& e12,
  ...
  mat3_t<T> operator*(const mat3_t<T>& other)
  {
    ...
    for (size_t i = 0; i < 3; i++)
    {
      for (size_t j = 0; j < 3; j++)
      {
        ...
        {
          result.data[i][j] = result.data[i][j] + this->data[i][k] * other.data[k][j];
        }
      }
    }
    return result;
  ...
  T trace() { return (*this).data[0][0] + (*this).data[1][1] + (*this).data[2][2]; }
  bool issymmetric() const
  {
    return isNear(this->data[0][1], this->data[1][0]) &&
      isNear(this->data[1][2], this->data[2][1]) && isNear(this->data[0][2], this->data[2][0]);
  }
#if 0
...
template <typename T>
const mat3_t<T> mat3_t<T>::identity = mat3_t<T>(
  T(1.0), T(0.0), T(0.0), T(0.0), T(1.0), T(0.0), T(0.0), T(0.0), T(1.0));
